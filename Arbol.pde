...
public class Nodo {
    ...
    public void graficarArista () {
      for (int i = 0; i < this.cantidadHijos(); i++) {
        float x = this.obtenerHijo(i).obtenerPosX() + (graficador.obtenerTamNodo() / 2);
        float y = this.obtenerHijo(i).obtenerPosY() + (graficador.obtenerTamNodo() / 2);
        
        motor.mostrarLinea(pos_x + (graficador.obtenerTamNodo() / 2), pos_y+ (graficador.obtenerTamNodo() / 2), x, y);
      }
